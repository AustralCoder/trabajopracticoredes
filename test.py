...
# 256 entradas precalculadas para el crc-16-ccitt (polinomio: 0x1021)
CRC_valores = [
...
def crc16_ccitt(data: bytes) -> int:
    ...
    for byte in data:
        ...
        # obtener indice para la tabla agarrando el byte mas alto del crc
        # y se le apica xor con el elnuevo byte

        tabla_indice = (crc >> 8) ^ byte #obtiene el valor mas alto del crc y lo "mezcla" con xor con el byte que esta procesando


        # actualizar el crc

        crc = ((crc << 8) & 0xFFFF ) ^ CRC_valores[tabla_indice] #desplaza el crc 8 bits a la izquierda dejando un hueco de 8 bits a la derecha
        # usa el valor 0xFFFF para asegurarse que siga teniendo solo 16 bits
        # ^ CRC_valores[tabla_indice] "mezcla" con xor el valor de la tabla en ese hueco

        return crc & 0xFFFF #DEVUELVE el resultado final en 16 bits 
...
def maybe_corrupt(data: bytes, p: float) -> bytes:
    ...
    if random.random() < p:
        ...
        byte_index = random.randint(0, len(dato_mutable) -1)
        # ahora elijo un valor aleatorio dentro de ese byte (0-7)
        bit_index = random.randint(0,7)

        # creo una mascara para voltear ese bit
